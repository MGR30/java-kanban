...
public abstract class TaskManagerTest<T extends TaskManager> {
    ...
    void setUp(T taskManager) {
        ...
        task = new Task("name", "description", NEW, TaskType.TASK, Duration.ofMinutes(20), LocalDateTime.now());
        taskManager.createTask(task);
        epic = new Epic("epicName", "epicDescription", NEW, TaskType.EPIC, Duration.ZERO, LocalDateTime.now());
        taskManager.createEpic(epic);
        subtask = new Subtask("subtaskName", "subtaskDescription", NEW, TaskType.SUBTASK, Duration.ofMinutes(20), LocalDateTime.now().plusMinutes(21));
    ...
    void epicStatusUpdateTest_AllSubtasksNew(){
        ...
        Subtask subtask1 = new Subtask("subtask1", "description", NEW, TaskType.SUBTASK, Duration.ZERO, LocalDateTime.now().plusMinutes(1));
    ...
    void epicStatusUpdateTest_AllSubtasksDone(){
        ...
        Subtask subtask1 = new Subtask("subtask1", "description", DONE, TaskType.SUBTASK, Duration.ZERO, LocalDateTime.now().plusMinutes(1));
        subtask1.setEpicId(epicId);
        taskManager.createSubtask(subtask1);
    ...
    void checkNotIntersectionTimeCorrect_TaskWithIntersectionTime(){
        Task newTask = new Task("taskName", "taskDescription", NEW, TaskType.TASK, Duration.ofMinutes(15), LocalDateTime.now().minusMinutes(10));
